...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        ...
        print(
            "Usage:\n {} {} {} {}\n".format(
                sys.argv[0], "folder_with_schemas", "generated_file_path", "name_prefix"
            )
        )
        sys.exit(-1)
    ...
    with io.BytesIO() as outf:
        ...
        out_text.write(
            "extern const std::vector<std::string> {}_schema_migrations = {{".format(
                prefix
            )
        )
        ...
        out_text.write(
            "extern const std::vector<std::string> {}_schema_rollback_migrations = {{".format(
                prefix
            )
        )
        if len(rollback_migrations_list) > 0:
            ...
            apend_migration(
                os.path.join(rollback_folder, rollback_migrations_list[-1]), out_text
            )
            out_text.write('"')
        out_text.write("\n};\n")
        ...
        out_text.write(
            'extern const std::string %s_current_schema = "%s";'
            % (prefix, current_schema_escaped)
        )
        out_text.write(
            "extern const int %s_current_schema_version = %u;"
            % (prefix, (len(migration_list) - 1))
        )
